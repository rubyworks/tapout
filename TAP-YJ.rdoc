...
a start document marker (`---`). Each document MUST have a +type+ field
which designates it a +header+, +case+, +test+, +note+ or +footer+. Any
...
equality, e.g. <code>assert_equal(4,3)</code>, +expected+ would be 3 and
+returned+ 4.
...
Lasty, the +time+ is the number of seconds that have elapsed since the
...
It MUST give the total test +count+ and +tally+ for each test status.
...
as a stream of JSON documents.
  ...
  {"type":"test","status":"pass","file":"foo.rb","line":"45","description":
  "an important test","returned":true,"expected":true,"source":"ok 1, 2",
  "snippet":[{44:"  ok 0,0"},{45:"  ok 1,2"},{46:"  ok 2,4"}],"time":0.01}
